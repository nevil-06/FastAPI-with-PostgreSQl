...
def get_competition(id: str, db: Session = Depends(get_db)):
    competiton = (
        ...
        .filter(Competition.id == id, Competition.is_deleted != true())
        .first()
    )
    return competiton
...
def create_competition(competiton: CompetitionTable, db: Session = Depends(get_db)):
    create_comptetition = (
        db.query(Competition)
        .filter(Competition.name == competiton.name, User.is_deleted != true())
        .first()
    )
...
# update
@competiton_router.put(
    "/competiton/{id}",
    status_code=status.HTTP_202_ACCEPTED,
    response_model=CompetitionResponse,
)
def update_competition(
    id: str, competiton: CompetitionTable, db: Session = Depends(get_db)
):
    updatecompetition = (
        db.query(Competition)
        .filter(Competition.id == id, Competition.is_deleted != true())
