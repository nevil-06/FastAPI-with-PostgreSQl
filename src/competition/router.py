...
def get_all_competitions(db: Session = Depends(get_db)):
    competitons = db.query(CompetitionDetails).filter(CompetitionDetails.is_deleted != True).all()
    return competitons
...
def get_competition(id: str, db: Session = Depends(get_db)):
    competiton = db.query(CompetitionDetails).filter(CompetitionDetails.id == id, CompetitionDetails.is_deleted != True).first()
    return competiton
...
def create_competition(competiton: CompetitionTable, db: Session = Depends(get_db)):
    create_comptetition= db.query(CompetitionDetails).filter(CompetitionDetails.name == competiton.name, UserDetails.is_deleted != True).first()
    
    if create_comptetition:
        return {"message": "Competiton details already exists"}
    else:
        new_competition = CompetitionDetails(
            id = competiton.id,
...
def update_competition(id: str, competiton: CompetitionTable, db: Session = Depends(get_db)):
    updatecompetition = db.query(CompetitionDetails).filter(CompetitionDetails.id == id, CompetitionDetails.is_deleted != True).first()
...
def delete_competition(id: str, db: Session = Depends(get_db)):
    deletecompetition = db.query(CompetitionDetails).filter(CompetitionDetails.id == id).first()
