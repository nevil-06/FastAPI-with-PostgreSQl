...
from src.entry.schema import EntryResponse, EntryTable
...
@entry_router.get(
    "/entries/all", status_code=status.HTTP_200_OK, response_model=List[EntryResponse]
)
...
@entry_router.get(
    "/entry/{id}", status_code=status.HTTP_200_OK, response_model=EntryResponse
)
...
@entry_router.get(
    "/entries/del", status_code=status.HTTP_200_OK, response_model=List[EntryResponse]
)
...
@entry_router.put(
    "/entry/{id}", status_code=status.HTTP_202_ACCEPTED, response_model=EntryResponse
)
